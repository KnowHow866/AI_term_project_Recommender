...
def test_cf_recommend_class():
    class CollaborativeFiltering(AlgorithmAbstraction):
        def __init__(self):
            self.session = DBManager.get_session()
            self.Food = pd.read_sql(session.query(Food).statement,session.bind)
            self.User = pd.read_sql(session.query(User).statement,session.bind)
            self.Review = pd.read_sql(session.query(UserRecommendationReview).statement,session.bind)
            self.food = pd.read_sql(session.query(Food).statement,session.bind).rename(columns={'id': 'food_id'})
            self.user = pd.read_sql(session.query(User).statement,session.bind).rename(columns={'id': 'user_id'})
            self.review = pd.read_sql(session.query(UserRecommendationReview).statement,session.bind)

        def _prediction_for_all(self):
            self.review['is_accept'] = np.where(self.review['is_accept']==True, 1, -1)
            rating_pivot = self.review.pivot(index='user_id',columns = 'food_id',values='is_accept').fillna(0)
        ...
        def _recommend_for_user(self, prediction_df, user_id, num_recommendations = 10):
            sorted_user_predictions = prediction_df.iloc[user_id - 1].sort_values(ascending=False)
            user_preference = pd.DataFrame(sorted_user_predictions).reset_index()
            user_preference.columns = ['food_id', 'ratings']

            user_review = self.review[self.review["user_id"] == user_id]
            food_not_yet_recommended = self.food[~self.food['food_id'].isin(user_review['food_id'])]
            recommend_list = pd.merge(food_not_yet_recommended, user_preference, on = "food_id").sort_values(by = ["ratings"], ascending = False).iloc[:num_recommendations]["food_id"].to_list()
        ...
        def _id_to_food_list(self, recommendation_list):
            result = self.session.query(Food).filter(Food.id.in_(recommendation_list)).all()
            food_map = {food.id: food for food in result}
            food_list = [food_map[id] for id in recommendation_list]
            return food_list
        ...
        def recommend(self, *args, **kwargs) -> 'Food[] , high recommendation prority in lower index ':
            ''' Everytime client ask for recommendation will invoke this method, return a python object '''
            user_id = 130
            prediction_df = self._prediction_for_all()
            recommendation_list = self._recommend_for_user(prediction_df, user_id)
            food_list = self._id_to_food_list(recommendation_list)
            return food_list
